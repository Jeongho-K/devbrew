...
넷(`adopted` · `rejected` · `held` · `applied`)의 재결정은 계속 거부한다 — `adopted` 는 아직 이번
라운드의 permit 이 관측되기를 기다리는 중이라 재결정할 것이 없고(다음 라운드가 `applied` 나
`expired` 로 답한다), 나머지 셋은 이미 누군가 의무를 졌거나 소멸했다. **사후(`post`) 만료**의 재결정에서 「채택」은 원복 의무를 관측 없이
종결한다는 뜻이므로, 게이트 텍스트가 그 뜻을 그 항목 옆에 밝힌다.

**알려진 한계 셋(PR 2 가 고칠 것).** 최종 리뷰가 실제 CLI 시퀀스로 셋 다 재현했다 — 호출자가
0 인 지금은 도달 불가지만, 첫 호출자가 붙는 순간부터 유효한 결함이다.

**한계 (a) — 재상승 후속의 「보류」.** `cmd_decide` 의 「보류」 거부는 `d["state"] == "expired"`
일 때만 발동한다. 그런데 전방 포인터는 의무를 **옮긴다** — `finalize` 가 `superseded_by` 를
쓰면 원본은 `blocked_expired`(위 술어)에서 빠지고, 남는 유일한 차단자는 후속인데 후속은
`record_findings` 가 만드는 평범한 `state: "open"` `decide` 다. 거부 가드는 `expired` 만
겨누므로 「보류」가 그 후속에는 그대로 통과한다 — `gate_summary` 의 `approval_ready` 가
True 로 뒤집히고 `render_gate` 는 아무것도 보여주지 않는다: 보류된 `decide` 는 `st["asks"]`
에 `blocks: []` 로 들어가는데 게이트가 그리는 것은 `blocking_ask_open`(「`blocks` 가 있어야
든다」) 뿐이다. `is_open()` 의 `ask` 분기도 같은 `blocks` 조건이라, 다음 라운드에도 돌아오지
않는다. §8.1·위 두 단계가 여는 「채택 / 기각 / 보류」선택지와 `_decision_view` 가 모든
`decide` 에 무조건 붙이는 `alternatives: ["채택(적용)", "기각(원복)", "보류"]` 는 이 재상승
후속을 예외로 두지 않는다 — §8.2 도 이 예외를 적는다(아래).

**한계 (b) — 사후 원복 의무의 강등.** `_auto_decides` 는 재상승으로 만든 후속에
`"kind": "pre"` 를 하드코딩한다. 원본이 `post`(얼림 diff 가 만든 결정)였고 사용자가
「기각」해 `revert` `permit`(`expect_hash` 에 기각 시점 스냅샷 해시)이 열렸던 경우, 그 permit
이 다음 라운드에 관측되지 못해 재상승한 후속은 `pre` 로 태어난다 — 「채택」이
`kind: "apply"` permit(`expect_hash` 없음)을 연다. `cmd_observe_diff` 는
`permit["kind"] == "apply"` 면 「`apply_anchors` 가 하나라도 diff 에 닿았는가」만 보고,
`revert` 였다면 봤을 「해시가 기각 시점 스냅샷과 같은가」는 보지 않는다 — 되돌리지 않은
얼림 위반이 그대로 승인된다. `render_gate` 의 사후 고지 꼬리(`d.get("kind") == "post"` 일
때만 붙는 문장)도 이 경로에서는 나오지 않는다. 하드코드 자체는 이 PR 이전부터 있었지만,
이 PR 의 누적(AC21)이 재상승을 항상 성공시키고 전방 포인터(AC20)가 원본의 차단을 항상
풀어 노출을 넓혔다.

**한계 (c) — `escalate` 된 `fix` 가 비차단·비가시.** `gate_summary` 의 `unapplied_fix` 는
`pending`/`intent_passed` 만 센다 — `escalated` 는 그 집합에도, `render_gate` 가 그리는 어떤
목록에도 없다. AC23 이전에는 `fix_anchors: ["*"]` 아래 미해소 앵커가 `fix_allowed` 로
분류돼 check-intent 를 통과하고 `intent_passed` 로 영구히 차단했는데, AC23 이 그 결함류를
다른 일반-경로 거부와 같은 `escalate` 경로로 돌리면서 새 결함류가 **비차단**이 됐다. AC23 의
근거(「단순 거부면 그 `fix` 가 미적용으로 남아 영구히 승인을 막는다」)는 옳지만, 고른
대안이 전제하는 「다음 라운드」는 보장되지 않는다.

**공통 뿌리 — 상태 축의 열거.** 셋을 따로 patch 하면 whack-a-mole 이다. `gate_summary` 의
차단 술어는 상태의 **열거**이고 `render_gate` 는 그 열거의 진부분집합만 그린다 — 이것은 이
절이 이미 처분 어휘에 대해 경고한 것과 같은 실패(「처분 어휘가 늘면 그 열거가 낡고 같은
fail-open 이 이름만 바꿔 재발한다」)가 상태 축에서 재발한 것이다. PR 2 는 (a)(b)(c) 를 각각
patch 하는 대신 「막는 집합」과 「렌더되는 집합」을 한 곳에서 도출하고 그 차이가 항상
공집합임을 락으로 걸어야 한다.
...
**예외 — 재상승 후속의 「보류」.** 열린 `decide` 가 재상승 후속(만료 항목의 `superseded_by` 가
가리키는 그 id)이면 「채택 / 기각 / 보류」 중 「보류」가 원본의 차단을 대신 넘겨받지 않는다 —
원본은 이미 `superseded_by` 로 `blocked_expired` 를 벗어났고, 후속을 보류하면 그 자리마저
차단·렌더 양쪽에서 빠져 승인 게이트가 조용히 열린다(§6.4 알려진 한계 (a)). PR 2 가 가드를
놓기 전까지 이 경로에서는 「보류」를 권하지 않는다.
...
| S17 | 심볼릭 링크 배포 러너의 수집기 모집단 수정을 PR 2 가 아니라 **1b** 에서 한다 | 보안 락의 모집단 변경은 위험 창(첫 호출자)이 열리기 **전에** 끝내는 편이 낫고, PR 2 의 diff 는 삭제 전수·껍데기화·major bump 가 겹쳐 회귀 귀속이 안 된다 · 1b 는 선재 RED 가 **둘**(`test_no_write_matcher_hooks_repo.sh`·`test_skill_orchestration_behavior.sh` — 후자는 비재귀 글롭이 놓쳤다가 R6 로 baseline 에 편입)뿐인 baseline 위에서 돈다 | 「PR 2 로」 — 두 수집기 수정이 1b 의 다른 변경과 회귀를 섞기 시작하면 |
