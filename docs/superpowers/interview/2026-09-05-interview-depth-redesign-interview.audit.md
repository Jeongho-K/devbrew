...
### 4.9 초안 v2 → v3 반영 (라운드 4 사용자 확인 후)

사용자 확인 넷 반영(seed 가벼운 구조=범위 안 · 비용 4범주 · a=한 브랜치 · B 는 대가를 알고 선택) + 역할 분담(interview 조사 / brainstorming 설계 / writing-plans 계획 / 그 다음 구현). critic r2 반영: 1(조건절 삭제) · 2(«Phase 0 제안:») · 3·4(예시 표시) · 5(handoff 대상 복원) · 6(«접근법 비교는 brainstorming 몫») · 7(반박 초청 명시) · 8(사례 탐색 비용 caveat) · 9(원문 인용 + a 정의) · 10(후행 결정으로 기술, «세션 고정 안 함»의 의미만 미확인) · 11(«찾지 못했다») · 12(«Phase 0 착안:») · 13(사례 경로 병기). codex r3 3건은 사용자 확인으로 해소. 냉독 r2 반영: 워크트리 요구 «범위 안» 명시.

### 4.10 codex r4 (초안 v3) — findings 0건. codex_failed: false.

### 4.11 seed-critic r3 (초안 v3) — 7건, 미반영(게이트 텍스트에 공시)

1. 축4 «사용자는 Phase 0(framing-requests) 쪽이 더 좋아 보인다고 했다» — 원문 «framing-requests 이쪽이 더 좋은거 같아»의 지시 대상이 (a) 질문 방식인지 (b) skill 이름인지 원문만으로 갈리지 않음. 대비를 제시한 것은 에이전트. → 원문 인용 + 미확인으로.
2. 축4 «인터뷰의 조사가 설계 입력으로 충분해지면 되고» — 원문 «최대한 … 양질의 정보»를 «충분»으로 재해석. → 원문 표현으로.
3. 축4 «재검증한다» «다시 연다» — 원문은 «할 수 있어야 한다»(능력)인데 의무형으로. → 양태 복원.
4. 축2 «audit 이 먼저 그 브랜치에 커밋돼 있어야» — audit 이 필수 선행으로 굳음. → 예시 지위.
5. 축3 «brief·설계·CHANGELOG 에서» — 출처 셋은 에이전트 열거. → 예시로.
6. 축3 «Phase 0 제안: 같은 결론이면 … 설명할 수 있어야 한다» — 완료 조건처럼 읽힘. → 낮추기.
7. 축3 «대리 조건이 필요할 수 있다» — 해법 형태 선점(경미).
- 축 밖: 라운드 0 추가 지시 «더 개선사항이 있거나» 절 누락.

### 4.12 냉독 r3 (초안 v3, opus)

이해: 순서(원인→비교→재설계→구현) 못 박힘; 세 겹(본체 + seed 구조 + Phase 0 워크트리, 둘 다 범위 안·주인공 아님); 핵심 = 종료 술어 이벤트→충족도; 방향 자체도 검증 대상; 피할 실패 = 형식·원장만 무거워짐; 두 번째 사이클 조건; 비용 열림·사례 탐색만 예외. 신경 쓰는 것: 같은 자리 재방문 · 원인 후보를 확정으로 팔지 않음 · 사용자 말과 추론 분리 · 닫지 말았어야 할 것을 닫은 사례 · 성공을 질문 수로 안 잼 · 경계. 모르겠는 곳(인터뷰가 열 질문): 충족도 판정자·근거 · «얕다»의 관측 근거와 원인 후보의 연결 · «질문 소진»의 두 해석(목록 소진 vs 떠오르지 않음) · framing-requests 방식이 무엇인지 · 추천 답안의 지위 · 어디까지 흔들 수 있나(원인이 하류에 있으면?) · seed 구조의 «가벼운» 정도와 결정 시점 · 워크트리 두 문장의 처분 · 형제 워크트리가 결정 항목인지 · 세션 종점 · 대리 조건이 산출물인지 · 측정 부재의 처방 · audit_file 역할 · 전제 어휘(derived·4-block·teach-beat·probe·round·상태 토큰).
