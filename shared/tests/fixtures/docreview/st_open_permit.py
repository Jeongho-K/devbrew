...
다시 채택한다 — Task 4 의 재결정 탈출구는 **모든** `cmd_decide` 호출에서 그 id 의
미소비 예약을 새 permit 을 열기 «전에» 폐기하므로(§`docreview_state.cmd_decide`,
설계 §6.4 상호배제의 절반), 이 원안은 첫 예약을 스스로 지워버려 dedup 이 막아야
